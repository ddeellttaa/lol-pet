Create a single horizontal sprite strip for the Codex app digital pet `lol-aphelios` in the state `waiting`.
...
Output exactly 6 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: tiny chibi Aphelios, silent Lunari marksman, side-swept dark hair, pale focused face, dark navy-black coat, muted violet shoulder mantle, moon-silver armor, compact crescent-shaped moon gun/blade held close, tiny cyan-blue Lunari glow, no separate Alune and no all-five-weapons clutter.
...
Animation action: Aphelios W cue, Phase weapon-swap waiting loop. Show a calm two-weapon swap stance: the crescent weapon shifts subtly between hands while a tiny off-hand barrel stays tucked close to the body.


State-specific requirements:
- The row must read as Phase in pet scale through body pose and a small close-in weapon swap only.
- Do not draw UI swap arrows, floating weapon wheels, text labels, inventory icons, ammo bars, detached weapon symbols, or a separate Alune figure.
- Keep both weapon hints attached to Aphelios' hands/body and inside the same frame slot.
