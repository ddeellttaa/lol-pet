Create a single horizontal sprite strip for the Codex app digital pet `lol-aphelios` in the state `waving`.
...
Animation action: Aphelios Q cue, compact Weapon Abilities gesture. Raise the crescent gun/blade slightly like a greeting, with one tiny hard-edged moonshot spark physically attached to the weapon, then return to neutral.
...
- The row must read as Aphelios' Q in pet scale: a restrained weapon-ready wave/aim pose, not a normal cheerful wave.
- Do not draw five separate weapons, detached bullet trails, target marks, sentries, enemy targets, skill icons, or UI.
