Create a single horizontal sprite strip for the Codex app digital pet `syndra` in the state `waving`.
...
Animation action: Syndra Q cue, Dark Sphere conjuring gesture. One hand lifts a small dark sphere close to the palm with a tiny hard-edged violet rim physically attached to the sphere, then returns toward neutral.


State-specific requirements:
- The row must read as Dark Sphere in pet scale through a hand pose and one close attached sphere only.
- Show the four frames as hand low, sphere forming close to palm, sphere held slightly higher, and return toward neutral.
- Do not draw giant orbs, detached projectile trails, multiple loose spheres, ground area markers, skill icons, text, wave marks, motion arcs, lines, sparkles, symbols, or floating effects around the hand.
