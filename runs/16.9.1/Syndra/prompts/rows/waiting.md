Create a single horizontal sprite strip for the Codex app digital pet `syndra` in the state `waiting`.
...
Animation action: Syndra W cue, Force of Will controlled waiting loop. Show a calm telekinetic stance with the close dark sphere tucked above both hands as if held by will, bobbing subtly with the body.


State-specific requirements:
- The row must read as Force of Will in pet scale through a restrained two-hand holding pose and one close attached sphere.
- Keep the sphere tucked inside the body silhouette or touching the hands; it must not become a separate distant prop.
- Do not draw thrown minions, enemy objects, UI arrows, distant floating balls, telekinesis beams, loose debris, text, labels, or scenery.
