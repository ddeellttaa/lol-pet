Create a single horizontal sprite strip for the Codex app digital pet `syndra` in the state `review`.
...
Output exactly 6 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: tiny chibi Syndra, proud dark sovereign mage, huge silver-white hair, angular black-purple crown, sharp dark eyes, dark violet bodysuit, jagged high shoulder armor, one small dark purple sphere held close to her hands, tiny hard-edged violet magic attached to the sphere, compact floating stance.
...
Animation action: Syndra R cue, Unleashed Power focus loop. Show a proud sovereign casting pose with two or three tiny hard-edged dark spheres clustered tight around the hands/body.


State-specific requirements:
- The row must read as Unleashed Power in pet scale through narrowed eyes, proud casting posture, and two or three tiny attached spheres clustered tight around the body.
- Keep every sphere touching or overlapping the pet silhouette or hands; do not let any sphere float far enough to become a separate sprite component.
- Do not draw a full-frame barrage, separate orbiting planets, enemy champion, execution marker, full-screen void, large black hole, magnifying glasses, papers, code, UI, punctuation, symbols, text, or scenery.
