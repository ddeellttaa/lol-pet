Create a single horizontal sprite strip for the Codex app digital pet `lol-neeko` in the state `running`.
...
Output exactly 6 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: # Neeko Pet Brief
